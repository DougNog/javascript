...
function validarCampos() {
  ...
  // Validação para garantir que o nome da pizza não esteja vazio
  if (nome === "") {
    document.getElementById("nome-erro").textContent =
      "Por favor, insira um nome para a pizza";
    valido = false;
  } else {
    document.getElementById("nome-erro").textContent = "";
  }

  // Validação para garantir que os ingredientes não estejam vazios
  if (ingredientes === "") {
    document.getElementById("ingredientes-erro").textContent =
      "Por favor, insira os ingredientes";
    valido = false;
  } else {
    document.getElementById("ingredientes-erro").textContent = "";
  }

  // Validação para garantir que o preço seja um número válido e maior que zero
...
function removerPizza(index) {
  if (
    confirm(`Tem certeza que deseja remover a pizza ${pizzas[index].nome}?`)
  ) {
    pizzas.splice(index, 1); // Remove a pizza do array
...
function buscarPizzaParaAlterar() {
  const termo = document
    ...
    .value.toLowerCase();
  const pizzaEncontrada = pizzas.find((p) =>
  ...
  if (pizzaEncontrada) {
    ...
    document.getElementById("busca-alterar-feedback").textContent = termo
...
function removerDoCarrinho(index) {
  pedidoAtual.itens.splice(index, 1); // Remove a pizza do carrinho
  pedidoAtual.total = pedidoAtual.itens.reduce(
    (sum, item) => sum + item.preco,
    0
  );
...
function atualizarResumoPedido() {
  ...
  total.textContent = pedidoAtual.total.toFixed(2); // Exibe o total atualizado
}
...
function finalizarPedido() {
  ...
  // Verifica se os campos do cliente estão preenchidos corretamente
  if (!nomeCliente || !enderecoCliente || !telefoneCliente) {
    alert("Por favor, preencha todas as informações do cliente");
    return;
  }

  // Finaliza o pedido com as informações do cliente
  pedidoAtual.cliente = {
    nome: nomeCliente,
